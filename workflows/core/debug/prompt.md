...
6. Use the normal Agent Runner continue control only after the user says they are done: type `/next` and press Enter, or use Ctrl-].
...
- If the user says they are done, use the normal Agent Runner continue control to end the workflow successfully: type `/next` and press Enter, or use Ctrl-].
