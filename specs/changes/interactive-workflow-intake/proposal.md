...
- Expose **`{{intake_handoff}}`** as a built-in template variable defined on **every** run: the absolute path of the sealed handoff on an intake launch, and the empty string on every direct or headless launch. A run resumes with the same value it originally had, so a resumed intake child still sees its sealed handoff.
- **Reserve `intake_handoff`** as a parameter and capture name. Built-ins otherwise have the lowest interpolation precedence, so a workflow declaring a parameter of that name would silently replace the sealed path and break the provenance guarantee the feature rests on.
- Update the first agent prompt in **`core:define-change`** and in `spec-driven:simple-change` to read `{{intake_handoff}}` when it is non-empty. `core:define-change` is shared by both the spec-driven and OpenSpec change flows, so both gain the behavior.
...
- `builtin-vars`: Add `{{intake_handoff}}`, which unlike existing built-ins is defined unconditionally, including as the empty string, and is reserved against workflow parameters and captures.
...
- `workflow-pre-validation`: Recognize `intake_handoff` as a built-in so prompts referencing it are not reported as undefined variables.
...
- Sub-workflow and loop execution contexts must propagate `{{intake_handoff}}` so nested steps in the launched workflow can reference it. Both context constructors copy run-scoped fields explicitly, so this is a deliberate addition rather than automatic.
