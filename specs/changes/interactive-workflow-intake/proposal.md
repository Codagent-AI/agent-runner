...
- Add **`agent-runner step submit-route`**, a fixed, argument-free control command. Before invoking it, the agent writes one typed JSON route request containing the workflow, parameters, and concise handoff text to a runner-provided path.
...
- On acceptance, the runner seals the route and handoff text into a run-owned sidecar. The sealed route records the **exact resolved workflow source path**, not just the canonical name, so the definition that was validated is the definition that runs.
...
- Expose **`{{intake_handoff}}`** as a built-in template variable defined on **every** run: the sealed handoff text on an intake launch, and the empty string on every direct or headless launch. More importantly, automatically prepend that text to the launched workflow's first agent prompt so handoff delivery cannot be omitted by a workflow. Bound the handoff to a prompt-safe size at submission and preserve it across resume.
...
- Remove workflow-specific handoff interpolation from **`core:define-change`** and `spec-driven:simple-change`; runner-level delivery applies uniformly to any routed workflow.
...
- **Define interruption and recovery.** A staged route persists in the sidecar across interruption, and a resumed intake attempt may replace it. If re-exec fails, the intake process is still alive and reports the failure. If the process is killed after the route freezes but before the launch, the intake run is already complete and cannot be resumed, so the user restarts intake; the sealed route and handoff remain preserved.
- **Exploration-only is a success path.** If the intake agent completes its step with no accepted route, the intake run finishes cleanly and launches nothing.
...
- `resume-by-session-id`: Preserve a run's original intake provenance across resume, so a resumed intake child restores its sealed handoff contents and a resumed intake parent restores its staged route.
...
**Route submission is one file behind a fixed command.** The agent writes its choice and handoff text to a JSON file at a runner-provided path and then runs a command whose text never varies.
...
env:  AGENT_RUNNER_ROUTE_REQUEST=<run-dir>/route-request.json
      ...
      { "workflow": "spec-driven:change",
        ...
        "handoff":  "Goal and settled context for the next agent." }
...
**The handoff travels in prompt context, with a reserved built-in retained for explicit use.** Runner-level prompt delivery keeps target workflow interfaces clean and prevents a workflow from accidentally dropping the handoff. `{{intake_handoff}}` remains defined on all runs—empty when there is no intake—and reserved against parameters and captures.

**Sealing and freezing close the gap between validation and use.** The handoff is part of the strict, bounded route request and is persisted with the resolved workflow and parameters. Freezing at completion acceptance closes the converse race, where a submission lands after the runner has already decided what it is launching.
...
- A new internal package owns the intake-route sidecar: its schema, atomic writes, and `staged → frozen` transitions. Keeping it separate from `internal/stateio`'s `RunState` is what prevents `writeStepState` from clobbering it.
...
- `internal/runner/` records parent provenance and the sealed handoff contents in the launched run's state, and restores both on resume. Resume currently copies an explicit field list, so these need deliberate addition.
