...
- Covers: `builtin-vars` — the `intake_handoff` built-in and built-in
...
- Setup: fixture workflows under `testdata/` — one whose prompt references the handoff built-in, one
  declaring a parameter of that name, one capturing into it; sealed handoffs both under and over the
  inline limit
...
- Assertions: the referencing workflow pre-validates and, when launched from intake, interpolates
  `{{intake_handoff}}` to the sealed handoff's contents, and that no variable exposes the file those
  contents came from; it resolves to the empty string on a direct run, without an unresolved-variable
  ...
  rejected at load and reported by pre-validation as a reserved name
