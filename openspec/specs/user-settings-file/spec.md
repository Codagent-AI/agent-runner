...
- **THEN** load succeeds and recognized keys are exposed to callers
...
- **THEN** load returns an empty settings value (equivalent to a missing file); the caller may treat this as "settings need to be (re)written"
