...
The user-settings editor SHALL present every user-editable setting from `~/.agent-runner/settings.yaml` (as defined by the `user-settings-file` capability) and SHALL NOT present lifecycle keys that the app manages on the user's behalf. The set of user-editable settings SHALL be `theme`, `autonomous_backend`, and `autonomous_permission_mode`, presented in that order. Lifecycle keys under `setup` and `onboarding` SHALL NOT be presented.
...
#### Scenario: Autonomous Permission Mode is presented

- **WHEN** the editor is open
- **THEN** the editor renders an `Autonomous Permission Mode` field with the two choices `Conservative` and `YOLO`, appearing below the Autonomous Backend field

#### Scenario: YOLO option carries risk copy

- **WHEN** the editor is open with the cursor on the `YOLO` option
- **THEN** the editor displays risk copy for the YOLO mode
...
- Up / Down SHALL move through all options across all fields in a flat list (Light, Dark, Headless, Interactive, Interactive for Claude, Conservative, YOLO). Up from the first option wraps to the last; Down from the last wraps to the first.
...
#### Scenario: Arrow keys move from backend to permission mode

- **WHEN** the editor is open with `Interactive for Claude` selected and the user presses Down
- **THEN** the option cursor moves to `Conservative` (the first option of the next field)

#### Scenario: Arrow keys wrap around

- **WHEN** the editor is open with `YOLO` selected (last option) and the user presses Down
...
#### Scenario: Save with an autonomous permission mode change

- **WHEN** the user opens the editor with `Conservative` selected for Autonomous Permission Mode, moves to `YOLO`, and presses Enter
- **THEN** `~/.agent-runner/settings.yaml` is written with `autonomous_permission_mode: yolo`, subsequent autonomous agent steps started after save use the new mode, and the editor closes
...
### Requirement: Autonomous Permission Mode field pre-selection reflects the currently persisted value

When the editor opens, the cursor on the `Autonomous Permission Mode` field SHALL be pre-selected to the value currently persisted in `~/.agent-runner/settings.yaml`. When the key is absent (defaulting to `conservative`), the `Conservative` option SHALL be pre-selected.

#### Scenario: Persisted permission mode is yolo

- **WHEN** the editor opens and `~/.agent-runner/settings.yaml` contains `autonomous_permission_mode: yolo`
- **THEN** the `YOLO` option is pre-selected

#### Scenario: Persisted permission mode is absent

- **WHEN** the editor opens and `~/.agent-runner/settings.yaml` does not contain an `autonomous_permission_mode` key
- **THEN** the `Conservative` option is pre-selected

#### Scenario: Re-opening after a mid-session change reflects the new value

- **WHEN** the user opens the editor, changes autonomous permission mode from `Conservative` to `YOLO`, saves, and then re-opens the editor
- **THEN** the `YOLO` option is pre-selected on the second open
