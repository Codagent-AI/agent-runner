...
Agent Runner SHALL emit `sub_workflow_start` before executing a sub-workflow's steps and `sub_workflow_end` after all sub-workflow steps complete, nested between the sub-workflow step's `step_start` and `step_end`.
