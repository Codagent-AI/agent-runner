...
When child findings inform a user-facing decision, the lead SHALL report each material child finding
separately from the lead's assessment, including findings the lead rejects or cannot verify. The
findings report MUST preserve each finding's rationale, evidence, and recommendation. The lead
assessment MUST state agreement, partial agreement, disagreement, or unable to verify, with reasons
and its own recommended action. The lead MUST NOT omit a material finding merely because it considers
the finding unsupported, invalid, or out of scope, or reduce it to a disposition label.
...
- **THEN** the user receives the original finding and recommendation plus the lead's verification gap
  and recommendation not to treat it as established
