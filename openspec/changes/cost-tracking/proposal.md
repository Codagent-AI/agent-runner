...
- **Claude**: headless invocation gains `--output-format json`; the result payload carries `usage` (input, output, cache-creation, cache-read) and `total_cost_usd` / per-model `costUSD` (Claude Code's own client-side estimate).
...
- **Behavioral side effect:** Claude headless steps switch from plain-text to JSON output, so the adapter must now filter the plain-text response out of the JSON envelope for capture variables and display (as Codex already does).
