...
- `tree.StepNode` (`internal/runview/tree.go`) gains `Attempts []AttemptMetrics` where `AttemptMetrics{Attempt int, Usage *model.UsageRecord, CostUSD *float64, DurationMs *int64, Outcome string}`. `applyStepEnd` (`internal/runview/audit.go`) **appends** an attempt instead of overwriting (adjusting `Tree.ApplyEvent`'s step re-execution replacement for metrics purposes; other runtime fields keep today's latest-wins behavior).
