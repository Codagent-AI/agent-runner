...
No agent-path parsing, sentinel, mouse, or hint code remains in the tree (`grep` for `AGENT_RUNNER_CONTINUE` finds nothing; the control channel's env vars are named `AGENT_RUNNER_CONTROL_*`); interactive shell steps still pass their existing spec behavior; the four hardening scenarios are covered by tests and passing; `make test`, `make lint`, the deterministic interactive harness, and all ten real-agent E2Es pass.
