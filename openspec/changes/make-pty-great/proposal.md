...
- A new `agent-runner step complete` command sends an authenticated completion event to the control endpoint, usable from inside the agent session or from another terminal as the universal fallback.
- All five supported adapters (Claude, Codex, Copilot, Cursor, OpenCode) get a natural completion surface (MCP tool, slash command, skill, or prompt instruction, per adapter) so the agent can advance the workflow the way `/next` used to.
...
- **BREAKING** (narrow): the Ctrl-] shortcut is removed — a universal hotkey requires reading the byte stream, which is exactly what is being given up. Its role (advance even when the agent is stuck) is covered by `agent-runner step complete` from another terminal, or by quitting the CLI and resuming. The output sentinel (`AGENT_RUNNER_CONTINUE_*` marker and legacy OSC form) also stops working, but agents receive the new completion instructions instead, so no user action is needed.
...
- `step-control-channel`: the out-of-band workflow control plane — per-run endpoint lifecycle, scoped per-step authentication, the `complete_step` event contract, the `agent-runner step complete` command, completion-instruction delivery to the agent, and per-adapter completion integration for all five supported CLIs.
...
- **Native terminal fidelity over a universal hotkey.** A parent cannot both grant full terminal ownership and intercept keys from the same stream. `/next` becomes an adapter/control-plane operation; `agent-runner step complete` from another terminal is the universal fallback.
...
- **Users**: interactive sessions look and advance the same — the agent finishes and the workflow moves on, and typed `/next` keeps working as a per-adapter command. What users gain is full native terminal behavior (mouse, paste, SS3, future protocols) with no Agent Runner patches; what they lose is Ctrl-].
