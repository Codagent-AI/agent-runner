# Capability: run-failure-evidence

## Purpose

Defines the structured failure record Agent Runner keeps when a deterministic check fails: what it contains, how the run's failure reason is derived from it, and where it is surfaced.
