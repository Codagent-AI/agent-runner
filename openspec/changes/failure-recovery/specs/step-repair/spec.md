# Capability: step-repair

## Purpose

Defines the `repair` block on shell and script steps: a bounded, resumable cycle in which a failed deterministic check triggers a repair, the same check reruns, and only the check's result decides whether the workflow continues.
