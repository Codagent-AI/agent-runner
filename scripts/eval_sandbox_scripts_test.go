...
func TestSandboxRunCanMountSubscriptionAuthFiles(t *testing.T) {
	...
	for path, body := range map[string]string{
		...
		filepath.Join(home, ".codex", "config.toml"):          "model = \"test\"\n",
...
func TestSandboxSyncHomeCopiesAllowedFilesWritable(t *testing.T) {
	...
	files := map[string]string{
		...
		filepath.Join(hostHome, "codex", "config.toml"):        "model = \"test\"\n",
	...
	for _, want := range []string{`model = "test"`, `[projects."/workspace/agent-runner"]`, `trust_level = "trusted"`} {
